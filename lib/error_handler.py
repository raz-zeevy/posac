...
class ErrorReporter:
    ...
    def __init__(self, gui=None, controller=None):
        ...
        if sys.platform == 'win32':
            ...
            self.bug_reports_dir = Path(os.path.expanduser("~")) / "AppData" / "Local" / "Posac" / "bug_reports"
        else:
            # For non-Windows platforms
            self.bug_reports_dir = Path(os.path.expanduser("~")) / ".local" / "share" / "Posac" / "bug_reports"
    ...
    def report_error(self, exception_info):
        ...
        if files:
            ...
            if not folder_opened:
                logger.warning(f"Could not open folder, will show path instead: {report_dir}")
            
            # Inform user where files are saved
            message = (
                f"Error report files have been saved to:\n"
                f"{report_dir}\n\n"
            )
...
def global_exception_handler(exctype, value, tb):
    ...
    if IS_PRODUCTION():
        ...
        report = messagebox.askquestion("ERROR: Application Failure", error_message, icon='error')
        ...
        if report == 'yes':  # askquestion returns 'yes' or 'no' instead of True/False
            ...
            messagebox.showinfo("Error Report Submitted", 
