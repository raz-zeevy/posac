...
class GenericCommand:
    def __init__(
        self, name, execute_fn, undo_fn, notebook, callback=None, *args, **kwargs
    ):
        self.name = name
...
def undoable(method):
    def wrapper(self, *args, **kwargs):
        ...
        undo_actions = {
            ...
            "remove_internal_variable": lambda: self._add_internal_variable(
                *args, **kwargs
            ),
            "add_external_variable": self._remove_external_variable,
            "remove_external_variable": lambda: self._add_external_variable(
                *args, **kwargs
            ),
        }
...
class PosacNotebook(tkinter.ttk.Notebook):
    def __init__(self, root, parent):
        ...
        self.add(self.external_variables_ranges_tab, text="External variables ranges")
    ...
    def bind_notebook_widgets(self):
        ...
        self.internal_variables_tab._on_toggle_row = self.update_posacsep_vars
        #
        ...
        old_validate_cmd = self.general_tab.lines_per_case_entry._validate_value_numeric
        new_validate_cmd = self.register(
            lambda p: self._validate_lines_per_case(p) and old_validate_cmd(p)
        )
        self.general_tab.lines_per_case_entry.config(
            validatecommand=(new_validate_cmd, "%P")
        )
    ...
    def get_state(self):
        ...
        return {
            ...
            "external_variables_ranges": self.external_variables_ranges_tab.get_all(),
    ...
    def _validate_lines_per_case(self, new_value):
        ...
        try:
            ...
            # Get all variable line numbers
            internal_lines = [
                int(row[1]) for row in self.internal_variables_tab.get_all_variables()
            ]
            external_lines = [
                int(row[1]) for row in self.external_variables_tab.get_all_variables()
            ]

            # If there are no variables, allow any valid number
            if not internal_lines and not external_lines:
                return True

            # Check if new value is less than any existing line numbers
            max_internal = max(internal_lines or [0])
            max_external = max(external_lines or [0])
