...
from lib.error_handler import ErrorReporter, global_exception_handler
from lib.utils import SET_MODE_TEST, IS_PRODUCTION

class TestErrorReporter:
    @pytest.fixture
    def error_reporter(self):
        """Create an ErrorReporter instance for testing"""
        SET_MODE_TEST()  # Ensure we're in test mode
        gui_mock = MagicMock()
    ...
    def test_report_error(self, error_reporter):
        ...
        with patch.object(error_reporter, 'capture_screenshot', return_value=True) as mock_screenshot, \
             patch.object(error_reporter, 'save_log', return_value=True) as mock_log, \
             patch.object(error_reporter, 'save_session', return_value=True) as mock_session, \
             patch.object(error_reporter, 'compose_email', return_value=True) as mock_email, \
             patch.object(error_reporter, 'open_folder', return_value=True) as mock_open_folder:
    ...
    def test_open_folder(self, error_reporter):
        ...
        try:
            ...
            with patch('os.startfile') if sys.platform == 'win32' else \
                ...
                # For macOS/Linux
                    ...
                    if sys.platform == 'darwin':
                        mock_open.assert_called_once_with(['open', temp_dir], check=True)
                    else:
                        mock_open.assert_called_once_with(['xdg-open', temp_dir], check=True)
