...
from lib.posac.recoding import apply_recoding, apply_recoding_operation, RecodingError
...
def test_sequential_value_recode():
    ...
    operation = RecodingOperation()
    operation.selected_variables = ['1']
    operation.recoding_pairs = [('1', '2'), ('2', '3')]  # If applied sequentially, 1 would become 3
    
    result = apply_recoding(data, [operation])
    expected = np.array([[2, 2], [3, 2], [3, 2]])  # 1->2, 2->3, but original 1 stays as 2
    assert np.array_equal(result, expected)
